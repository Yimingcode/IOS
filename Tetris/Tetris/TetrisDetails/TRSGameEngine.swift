...
class TRSGameEngine: NSObject {
    ...
    var gameStatus: TRSGameStatus = .Ready {
        didSet {
            if self.statusChanged != nil {
                self.statusChanged!(self.gameStatus)
            }
        }
    }
    ...
    var statusChanged: ((_ status: TRSGameStatus) -> ())? = nil
    ...
    func hitTimeTick() {
        if self.canMoveDown(shape: self.currentShape!) {
            self.currentShape!.moveDown()
        } else {
            self.shapeLayer!.receiveShape(shape: self.currentShape!)
    ...
    func genNextShape() {
        ...
        self.nextLayer?.setNextShape(newNext: self.nextShape!)
    ...
    func isGameOver() -> Bool {
        if self.isCollision(shape: self.currentShape!) {
            return true
        }
    ...
    func moveLeft() {
        if self.canMoveLeft(shape: self.currentShape!) {
    ...
    func moveRight() {
        if self.canMoveRight(shape: self.currentShape!) {
    ...
    func moveDrop() {
        while self.canMoveDown(shape: self.currentShape!) {
            self.currentShape?.moveDown()
        }
    ...
    func moveRotate() {
        ...
        let can = self.canRotateClockWise(shape: self.currentShape!)
        if can.can {
            self.currentShape?.clockwise(offsetLeft: can.offsetLeft, offsetTop: can.offsetTop, isTemp: false)
            
            // set shadow
            if self.showShadow {
                self.currentShadow?.clockwise(offsetLeft: can.offsetLeft, offsetTop: can.offsetTop, isTemp: true)
                self.currentShadow?.left = (self.currentShape?.left)!
                self.currentShadow?.top = (self.currentShape?.top)!
                self.setShadowPosition()
            }
    ...
    func setShadowPosition() {
        while (self.canMoveDown(shape: self.currentShadow!)) {
    ...
    func canRotateClockWise(shape: TRSShape) -> (can: Bool, offsetLeft: Int, offsetTop: Int) {
        ...
        let tempShape = shape.copyRotateShape(isClockWise: true)
        ...
        if minLeftInLayer < 0 {
            ...
            if self.isCollision(shape: tempShape) {
                isCan = false
            }
            else {
